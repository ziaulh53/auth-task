    ...
    <div class="flex items-center justify-center h-screen">
        <div class="w-[400px] border-2 p-5 shadow-lg bg-slate-100">
            <div class="mb-4">
                ...
                <input class="w-full border-2 px-4 py-2 rounded-md" name="email" v-model="state.name" />
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="password" class="w-full border-2 px-4 py-2 rounded-md" name="cpassword" v-model="state.cpassword" />
            </div>
            <div class="mb-2">
...
const state = ref({ name: '', password: "", email: "", cpassword: "" });
const loading = ref(false);

const disabled = computed(() => !state.value.email || !state.value.password || !state.value.name || (state.value.password !== state.value.cpassword));
...
const handleRegister = async () => {
    loading.value = true;
    try {
        const res = await userStore.userRegistration('signup', { ...state.value });
        if (res.success) {
